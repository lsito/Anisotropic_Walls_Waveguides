...
norm_factor = np.amax(np.amax(np.angle(A)))

for idx, el in enumerate(labels):
    row = idx // 3  # Determine the row (0 or 1)
    col = idx % 3   # Determine the column (0, 1, or 2)

    contour = ax[row, col].contourf(Fields_WR90.x, Fields_WR90.y, np.angle(A[idx])/norm_factor, cmap='viridis', levels=100)
...
fig.suptitle(f'Angle Mode Theory TE{1}{0}, WR90', fontsize=16)
...
A = [-1j*Fields_WR90.Ex, -1j*Fields_WR90.Ey, -1j*Fields_WR90.Ez, 
     -1j*Fields_WR90.Hx, -1j*Fields_WR90.Hy, -1j*Fields_WR90.Hz]
...
# Set a title for the whole figure
fig.suptitle(f'Angle Analytical TE{1}{0}, WR90', fontsize=16)
